 ...
 * produce ticking readings no network. The feed PAUSES when the device is
 * offline, the tab is hidden, or the user prefers reduced motion, which keeps it
 * honest (offline) and cheap on low-end devices.
 */
export function useSimulatedTelemetry(intervalMs = 4000): {
  ...
  const live = online && visible && !reducedMotion
