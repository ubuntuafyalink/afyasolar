...
const gradeColors: Record<string, string> = {
  A: 'border-transparent bg-success/10 text-success',
  B: 'border-transparent bg-secondary text-secondary-foreground',
  C: 'border-transparent bg-warning/15 text-warning-foreground',
    ...
    <>
      <div className="space-y-6">
        <div className="animate-in fade-in duration-300">
          <h1 className="text-2xl font-semibold tracking-tight text-foreground">Energy Efficiency</h1>
          <p className="text-sm text-muted-foreground mt-1">Efficiency scores, EUI, and improvements by facility</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 animate-in fade-in slide-in-from-bottom-2 duration-300 delay-75">
          ...
          <Card className="rounded-lg border-border shadow-sm">
            ...
            <CardContent>
              ...
              <p className="text-sm text-muted-foreground mt-1">of {facilities.length} sites</p>
            </CardContent>
          </Card>
          <Card className="rounded-lg border-border shadow-sm">
            <CardHeader className="pb-2">
            ...
            <CardContent>
              ...
              <p className="text-sm text-muted-foreground mt-1">kWh per month</p>
            </CardContent>
          </Card>
        </div>

        {/* Per-facility efficiency cards */}
        <Card className="rounded-lg border-border shadow-sm overflow-hidden animate-in fade-in slide-in-from-bottom-2 duration-300 delay-100">
          ...
          <CardContent className="p-0">
            <div className="max-h-[60vh] overflow-y-auto overflow-x-hidden scroll-smooth [scrollbar-gutter:stable]">
              {facilities.length === 0 && (
                <div className="p-6">
                  <EmptyState icon={<Gauge />} title="No facility data loaded." />
                </div>
              )}
              {facilities.map((f) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      'w-full text-left flex flex-wrap items-center gap-4 p-4 border-b border-border last:border-b-0 cursor-pointer',
                  ...
                  >
                    ...
                    <div className="flex items-center gap-4 flex-wrap">
                      <div className="text-center">
                        ...
                        <p className="text-lg font-bold text-foreground">{eff.score}</p>
                      </div>
                      ...
                      <div className="text-center" title="Energy Use Intensity: consumption per m² per month; lower is better.">
                        <p className="text-xs text-muted-foreground">EUI (now)</p>
                        <p className="text-sm font-medium text-foreground">{eff.euiAfter} kWh/m²/mo</p>
                      </div>
            ...
            <>
              ...
              <div className="space-y-6 pt-2">
                <div className="rounded-lg border border-primary/20 bg-primary/5 p-4 space-y-2">
                  ...
                  {meterLoading && <p className="text-xs text-muted-foreground">Loading meter insight…</p>}
                  ...
                  {!meterLoading && !meterInsight && (
                    <p className="text-xs text-muted-foreground">Could not load meter insight.</p>
                  )}
                </div>
                <div className="flex flex-wrap items-center gap-4">
                  <div className="rounded-lg border-2 border-border bg-muted p-6 text-center min-w-[120px]">
                    <p className="text-4xl font-bold text-foreground">{efficiencyMap[selectedFacility.id].score}</p>
                ...
                <div className="rounded-lg border border-border p-4 space-y-2">
                  <h4 className="text-sm font-semibold text-foreground">
                    ...
                    <span title="Energy consumption per square metre per month; used to compare similar facilities." className="text-muted-foreground cursor-help ml-1 inline-flex align-middle">
                      <HelpCircle className="w-3.5 h-3.5" aria-hidden />
                    </span>
                  </h4>
                  ...
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="font-medium text-primary">{efficiencyMap[selectedFacility.id].euiAfter} kWh/m²/month</span>
                  </div>
                ...
                <div className="rounded-lg border border-border p-4">
                  <h4 className="text-sm font-semibold text-foreground mb-2">Vs sector benchmark</h4>
                  <p className="text-sm text-muted-foreground">
                ...
                <div className="rounded-lg bg-muted border border-border p-4 flex items-start gap-3">
                  <Leaf className="w-5 h-5 text-success shrink-0 mt-0.5" aria-hidden />
                  <div>
                    <p className="text-sm font-medium text-foreground">Carbon impact</p>
                    <p className="text-sm text-muted-foreground">
