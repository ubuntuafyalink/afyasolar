 ...
 * with the SAME arguments (incl. the systemKw default of 5) so React Query
 * serves it from the forecast card's cache — keep the defaults in lockstep.
 */
export function ClimateOutlookReportCard({
  ...
  const forecast = useAiForecast({ lat, lon, horizon: "monthly", systemKw })
